...
"""
filename = "queen_of_Katoren_magic.txt"
def change_name(list_line, name = "Brunhild"):
    """
    Function that changes "<name>" for defined name.
    """
    k = 0#index that help us traverse list_line
    for t in list_line:
        if t == "<name>":
            list_line[k] = name #overwrite
        k+=1
    return list_line
with open(filename, "w+") as f:
    for line in string.splitlines():
        if "<name>" in line:
            ...
            l = change_name(l)
            l = " ".join(l)
            line = l #overwrite line with l
        f.write(line + "\n")




## Solución 2: uso de expresiones regulares (reguex)
#### Fuente de información: https://docs.python.org/3/library/re.html
import re
